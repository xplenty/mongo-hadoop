...
public class MongoLoaderTest {
    ...
    public void testSimpleChararray() throws Exception {
        ...
        //Test no schema
        pigServerLocal.registerQuery(format("A = LOAD 'mongodb://localhost:27017/%s.%s' using com.mongodb.hadoop.pig.MongoLoader();",
                                            dbName, "testSimpleChararray"));
        ...
        pigServerLocal.registerQuery(format("B = LOAD 'mongodb://localhost:27017/%s.%s' using com.mongodb.hadoop.pig"
                                            + ".MongoLoader('mongo_id:chararray, a:chararray', 'mongo_id');", dbName,
                                            "testSimpleChararray"));
        ...
        pigServerLocal.registerQuery(format("C = LOAD 'mongodb://localhost:27017/%s.%s' using com.mongodb.hadoop.pig"
                                            + ".MongoLoader('s:chararray', '', '--loadaschararray');", dbName, "testSimpleChararray"));
        ...
        assertEquals(format("{ \"_id\" : { \"$oid\" : \"%s\"} , \"a\" : \"%s\"}", obj1.getObjectId("_id"), obj1.getString("a")),
                     result1.get(0));
        assertEquals(format("{ \"_id\" : { \"$oid\" : \"%s\"} , \"a\" : \"%s\"}", obj2.getObjectId("_id"), obj2.getString("a")),
                     result2.get(0));
    }
    ...
    public void testEmbeddedObject() throws Exception {
        ...
        //Test Schema and idAlias
        pigServerLocal.registerQuery(format("B = LOAD 'mongodb://localhost:27017/%s.%s' using com.mongodb.hadoop.pig"
                                            + ".MongoLoader('mongo_id:chararray, f1:chararray, f2:int, f3:bag{}, "
                                            + "f4:(i1:chararray, i2:int)', 'mongo_id');", dbName, "testEmbeddedObject"));
